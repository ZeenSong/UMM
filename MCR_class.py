...
class MaximalCodingRateReduction(nn.Module): #TODO: fix this
    ...
    def discrimn_loss(self, Z):
        ...
        c = d / (m * self.eps)
        return logdet(c * Z.T @ Z + I) / 2.

    def compress_loss(self, Z, y):
        loss_comp = 0
        m, d = Z.shape
        I = torch.eye(d).to(Z.device) 
        for j in y.unique():
    ...
    def compress_with_prob(self, Z, pi, y):
        loss_comp = 0
        m, d = Z.shape
        I = torch.eye(d).to(Z.device)
        for j in y.unique():
            pi_j = torch.diag(pi[:,int(j)])
            # Z_j = Z[y == int(j)]
            # m_j = Z_j.shape[0]
            m_j = pi[:,int(j)].sum()
            c_j = d / (m_j * self.eps)
            logdet_j = logdet(I + c_j * Z.T @ pi_j @ Z)
            loss_comp += logdet_j * m_j / (2 * m)
        return loss_comp

    # def forward(self, Z, y, P):
    def forward(self, Z, y):
        Pi = y # TODO: change this to prob distribution
        loss_discrimn = self.discrimn_loss(Z)
        # loss_compress = self.compress_with_prob(Z, P, Pi)
        loss_compress = self.compress_loss(Z, y)
