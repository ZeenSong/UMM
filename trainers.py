...
def simclr_trainer(batch, model, opt, cfg, accelerator: Accelerator):
    model.train()
    opt.zero_grad()
    x1, x2 = batch
...
def swav_trainer(batch, model, opt, cfg, accelerator):
    model.train()
    opt.zero_grad()
    x1, x2 = batch
    x1, x2 = x1.cuda(), x2.cuda()
...
def moco_trainer(batch, model, opt, cfg, accelerator: Accelerator, K=4096, momentum=0.999):
    model.train()
    opt.zero_grad()
    x1, x2 = batch
    x1, x2 = x1.cuda(), x2.cuda()
...
def byol_trainer(batch, model, opt, cfg, accelerator: Accelerator):
    model.train()
    opt.zero_grad()
    x1, x2 = batch
    x1, x2 = x1.cuda(), x2.cuda()
...
def simsiam_trainer(batch, model, opt, cfg, accelerator: Accelerator):
    model.train()
    opt.zero_grad()
    x1, x2 = batch
    x1, x2 = x1.cuda(), x2.cuda()
...
def barlow_trainer(batch, model, opt, cfg, accelerator: Accelerator):
    model.train()
    opt.zero_grad()
    x1, x2 = batch
    x1, x2 = x1.cuda(), x2.cuda()
...
def vicreg_trainer(batch, model, opt, cfg, accelerator: Accelerator):
...
def wmse_trainer(batch, model, opt, cfg, accelerator: Accelerator, w_iter=1, w_size=128):
    model.train()
    opt.zero_grad()
    x1, x2 = batch
    bs = x1.shape[0]
...
def relic_trainer(batch, model, opt, cfg, accelerator: Accelerator, max_tau=5.0, ):
    model.train()
    opt.zero_grad()
    n = x.size(0)
    x, x_prime = batch
    x, x_prime = F.normalize(x, p=2, dim=-1), F.normalize(x_prime, p=2, dim=-1)
    x, x_prime = model(batch)
...
def mec_trainer(batch, model, optimizer, cfg, accelerator: Accelerator, m=1024):
    model.train()
    optimizer.zero_grad()
    x1, x2 = batch
    ...
    mec_loss = (mec_loss(p1, z2, model.scheduler[cfg.epoch]) + mec_loss(p2, z1, model.scheduler[cfg.epoch])) * 0.5 / cfg.m

    # scaled loss by lamda
    loss = -1 * mec_loss * model.lamda_inv
...
def corinfomax_trainer(batch, model, optimizer, cfg, accelerator: Accelerator):
    model.train()
    optimizer.zero_grad()
    x1, x2 = batch
    z1,z2 = model(x1,x2)
